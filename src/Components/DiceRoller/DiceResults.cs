...
namespace ChariotSanzzo.Components.DiceRoller {
	public class DiceResults {
		...
		public async Task<(bool wasSuccess, DiscordEmbed embed)> ToDiscordEmbedAsync(DiscordClient? client, ulong? userId) {
			DiscordEmbedBuilder embed = new();
			if (client != null && userId != null) {
				var user = await client.GetUserAsync((ulong)userId);
				if (user != null) {
